...
// 함수를 먼저 생성 후 이를 호출하기 때문에 원인/근거, 활용에대한 순서를 파악하기 쉽다.
console.log( Fn1(1,2) );
...
let my1 = new DataFn(1,'chowon', 'male'); //생성자 함수
console.clear();
console.log( my1 );
...
function UserData(key,id ,pw, user ,phone){
  this.key = key;
  this.id = id;
  this.password = pw;
  this.userName = user;
...
sendBtn.addEventListener('click',function(e){
  ...
  fninsertData(userId.value,userName.value,phoneNumber.value);
  userId.value = '';
  userName.value = '';
  phoneNumber.value = '';
